...
async function reserveStock(event) {
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      console.log(`[inventory] ⚠ Stock ya reservado para orderId=${orderId} (idempotencia). Ignorando.`);
    ...
    if (available < quantity) {
      await client.query('ROLLBACK');
      console.warn(`[inventory] Stock insuficiente para ${productId}: disponible=${available}, requerido=${quantity}`);

      await publish('shipments', orderId, {
        eventType: 'StockFailed',
        orderId, customerId, customerEmail, customerName,
        productId, productName, quantity, totalPrice, paymentId,
    ...
    console.log(`[inventory] Stock reservado: ${quantity}x ${productId} para orderId=${orderId} | disponible restante=${available - quantity}`);
