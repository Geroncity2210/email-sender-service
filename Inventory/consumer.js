...
async function startConsumer() {
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      ...
      console.log(
        `[inventory] ← Recibido de [${topic}] partition=${partition} offset=${message.offset}`,
      );
      ...
      if (event.paymentStatus !== "APPROVED") {
        console.log(
          `[inventory] Pago no aprobado para orderId=${event.orderId}, estado=${event.paymentStatus}. No se reserva stock.`,
        );
        return;
      }
